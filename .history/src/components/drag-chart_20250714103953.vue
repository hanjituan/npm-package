...
const barData = computed(() => {
  ...
  const filteredData = props.valueData.reduce((acc: string[], cur: string) => {
    ...
    return [...acc, dayjs(cur).set('minute', 0).set('second', 0).format('YYYY-MM-DD HH:mm:ss')]
  }, [])
...
const getChartOption = (): echarts.EChartsOption => {
  ...
  const datalist = barData.value.map(
    (item: string | number | Date | dayjs.Dayjs | null | undefined) => {
      const index = xAxisData.value.findIndex(x => dayjs(x).isSame(dayjs(item), 'hour'))
      return [index, 1]
    },
  )
    ...
    series: [
      {
        ...
        data: data.value,
        areaStyle: {},
        symbolSize: 0, // TODO: 待确认 // props.symbolSize,
      },
      ...seriesData,
    ],
  }
...
const getBorderStyle = (initialPoints: string | any[]): any => {
  return {
    ...
    data: [
      // 左侧竖线
      [{ coord: [initialPoints[0][0], 0] }, { coord: [initialPoints[0][0], initialPoints[0][1]] }],
      // 右侧竖线
      [
...
// 初始化图表
onMounted(() => {
  myChart = echarts.init(chartRef.value as HTMLElement, null, { renderer: 'svg' })
