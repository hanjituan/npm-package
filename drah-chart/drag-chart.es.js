import { getCurrentScope as j, onScopeDispose as _, computed as I, toValue as S, watch as H, shallowRef as $, getCurrentInstance as ee, onMounted as G, defineComponent as J, ref as te, onBeforeUnmount as ae, createElementBlock as ne, openBlock as ie, createElementVNode as oe } from "vue";
import m from "dayjs";
import * as se from "echarts";
const re = "data:image/png;base64,", Ae = "data:image/png;base64,", le = {
  // x轴的开始和结束时间
  timeRange: {
    type: Array,
    default: () => [m().subtract(1, "day"), m()]
  },
  // 开始图标
  startIcon: {
    type: String,
    default: re
  },
  // 结束图标
  endIcon: {
    type: String,
    default: Ae
  },
  // 拖拽点的大小
  symbolSize: {
    type: Number,
    default: 20
  },
  // 目前是有值的柱子
  valueData: {
    type: Array,
    default: () => []
  },
  // 当前时间范围
  activeTime: {
    type: Array,
    default: () => [0, 12]
  },
  // X轴的间隔
  interval: {
    type: Number,
    default: 4
  },
  // 是否自动计算间隔, 如果为true，则interval会被忽略
  autoInterval: {
    type: Boolean,
    default: !0
  },
  // 是否支持点击修改位置
  needClick: {
    type: Boolean,
    default: !0
  },
  // 最大选择范围（小时）
  maxRange: {
    type: Number,
    default: 2 * 24
    // 3天 = 72小时
  },
  // 最小选择范围（小时）
  minRange: {
    type: Number,
    default: 3
    // 3小时
  }
};
function ue(u) {
  return j() ? (_(u), !0) : !1;
}
const ce = typeof window < "u" && typeof document < "u";
typeof WorkerGlobalScope < "u" && globalThis instanceof WorkerGlobalScope;
const ve = ce ? window : void 0;
function Z(u) {
  var v;
  const t = S(u);
  return (v = t?.$el) != null ? v : t;
}
function de() {
  const u = $(!1), v = ee();
  return v && G(() => {
    u.value = !0;
  }, v), u;
}
function me(u) {
  const v = de();
  return I(() => (v.value, !!u()));
}
function ge(u, v, t = {}) {
  const { window: x = ve, ...g } = t;
  let s;
  const y = me(() => x && "ResizeObserver" in x), r = () => {
    s && (s.disconnect(), s = void 0);
  }, p = I(() => {
    const h = S(u);
    return Array.isArray(h) ? h.map((f) => Z(f)) : [Z(h)];
  }), a = H(
    p,
    (h) => {
      if (r(), y.value && x) {
        s = new ResizeObserver(v);
        for (const f of h)
          f && s.observe(f, g);
      }
    },
    { immediate: !0, flush: "post" }
  ), w = () => {
    r(), a();
  };
  return ue(w), {
    isSupported: y,
    stop: w
  };
}
const pe = { class: "chart-container" }, xe = J({
  name: "DragChart"
}), Re = /* @__PURE__ */ J({
  ...xe,
  props: {
    ...le
  },
  emits: ["update:activeTime"],
  setup(u, { emit: v }) {
    const t = u, x = v, g = te();
    let s;
    const y = (e = t.timeRange[0], n = t.timeRange[t.timeRange.length - 1]) => m(n).diff(e, "day") + 1, r = I(() => {
      const e = y();
      return console.log(e * 24), e * 24;
    }), p = I(() => {
      const e = [], n = m(t.timeRange[0]), i = y();
      for (let o = 0; o <= i; o++) {
        const A = n.add(o, "day");
        for (let l = 0; l < 24; l++)
          e.push(A.hour(l).format("YYYY-MM-DD HH:00:00"));
      }
      return e;
    }), a = I({
      get() {
        return w();
      },
      set(e) {
        x("update:activeTime", e);
      }
    }), w = () => {
      if (!t.activeTime || t.activeTime.length !== 2)
        return [
          [0, 0],
          [p.value.length - 1, 0]
        ];
      const e = p.value.findIndex(
        (i) => m(i).isSame(
          m(t.activeTime[0]),
          "hour"
        )
      ), n = p.value.findIndex(
        (i) => m(i).isSame(
          m(t.activeTime[1]),
          "hour"
        )
      );
      return [
        [e, 0],
        [n, 0]
      ];
    };
    ge(g, () => {
      s && s.resize({
        width: "auto",
        height: "auto"
      });
    });
    const f = I(() => Math.floor((r.value - 1) / 12) + 1), T = I(() => !t.valueData || t.valueData.length === 0 ? [] : t.valueData.reduce((n, i) => {
      const o = n[n.length - 1];
      return o && m(o).hour() === m(i).hour() ? n : [
        ...n,
        m(i).set("minute", 0).set("second", 0).format("YYYY-MM-DD HH:mm:ss")
      ];
    }, [])), W = (e, n) => {
      const i = [];
      for (let o = e; o <= n; o += 1)
        i.push([o, 2]);
      return i;
    }, z = (e, n) => {
      const i = Math.abs(n - e);
      if (i < t.minRange) {
        const o = (e + n) / 2, A = t.minRange / 2;
        e = Math.max(0, o - A), n = Math.min(r.value, o + A), n - e < t.minRange && (e === 0 ? n = Math.min(r.value, t.minRange) : n === r.value && (e = Math.max(0, r.value - t.minRange)));
      }
      if (i > t.maxRange) {
        const o = (e + n) / 2, A = t.maxRange / 2;
        e = Math.max(0, o - A), n = Math.min(r.value, o + A), e === 0 && n - e > t.maxRange ? n = t.maxRange : n === r.value && n - e > t.maxRange && (e = r.value - t.maxRange);
      }
      return {
        leftX: Math.round(e),
        rightX: Math.round(n)
      };
    }, C = () => {
      s.setOption({
        graphic: a.value.map((e) => ({
          position: s.convertToPixel("grid", e)
        }))
      });
    }, K = (e) => {
      s.dispatchAction({
        type: "showTip",
        seriesIndex: 0,
        dataIndex: e
      });
    }, F = () => {
      s.dispatchAction({
        type: "hideTip"
      });
    }, M = () => {
      const e = Math.min(a.value[0][0], a.value[1][0]), n = Math.max(a.value[0][0], a.value[1][0]), i = W(e, n), o = a.value[0][0], A = a.value[1][0], l = o < A;
      s.setOption({
        series: [
          {
            id: "a",
            data: a.value,
            z: 80,
            silent: !0,
            animation: !1,
            symbolSize: 0
          },
          {
            id: "b",
            data: i,
            type: "line",
            areaStyle: {
              color: "rgba(160,210,255,0.14)"
            },
            symbolSize: 0,
            markLine: X(i),
            z: 90,
            silent: !0,
            animation: !1
          }
        ]
      }), s.setOption({
        graphic: a.value.map((d, c) => ({
          position: s.convertToPixel("grid", d),
          type: "image",
          style: {
            image: (l ? c === 0 : c === 1) ? t.startIcon : t.endIcon,
            width: t.symbolSize,
            height: t.symbolSize,
            x: -t.symbolSize / 2,
            y: -t.symbolSize / 2
          },
          invisible: !1,
          z: 100
        }))
      });
    }, Q = () => {
      x(
        "update:activeTime",
        a.value.map((e) => p.value[e[0]])
      );
    }, N = (e, n) => {
      const i = s.convertFromPixel("grid", n);
      i[0] = Math.round(Math.min(Math.max(i[0], 0), r.value)), i[1] = a.value[e][1], a.value[e] = i;
      const o = Math.min(a.value[0][0], a.value[1][0]), A = Math.max(a.value[0][0], a.value[1][0]), { leftX: l, rightX: d } = z(
        o,
        A
      ), c = a.value[0][0] <= a.value[1][0] ? 0 : 1, D = c === 0 ? 1 : 0;
      a.value[c] = [l, 0], a.value[D] = [d, 0], M(), Q();
    }, Y = (e, n) => {
      const i = s.convertFromPixel("grid", n);
      i[0] = Math.min(Math.max(i[0], 0), r.value), i[1] = a.value[e][1], a.value[e] = i, M();
    }, q = (e) => {
      if (!e.ctrlKey && !e.shiftKey) return;
      e.preventDefault();
      const n = e.deltaY, i = Math.abs(a.value[1][0] - a.value[0][0]);
      if (e.shiftKey) {
        const b = n > 0 ? 2 : -2;
        let R = a.value[0][0] + b, E = a.value[1][0] + b;
        R < 0 && (R = 0, E = i), E > r.value && (E = r.value, R = r.value - i), a.value[0] = [R, 0], a.value[1] = [E, 0], M(), Q();
        return;
      }
      const o = 0.1;
      if (i <= t.minRange && n > 0 || i >= t.maxRange && n < 0) return;
      let A = n > 0 ? Math.min(
        t.maxRange,
        Math.min(r.value, i * (1 + o))
      ) : Math.max(t.minRange, i * (1 - o));
      const l = (a.value[0][0] + a.value[1][0]) / 2;
      let d = Math.round(l - A / 2), c = Math.round(l + A / 2);
      d < 0 && (d = 0, c = Math.min(r.value, Math.round(A))), c > r.value && (c = r.value, d = Math.max(0, Math.round(r.value - A)));
      const { leftX: D, rightX: B } = z(
        d,
        c
      );
      a.value[0] = [D, 0], a.value[1] = [B, 0], M(), Q();
    }, L = () => {
      const e = Math.min(a.value[0][0], a.value[1][0]), n = Math.max(a.value[0][0], a.value[1][0]), i = W(e, n), o = T.value.map(
        (l) => [p.value.findIndex(
          (c) => m(c).isSame(m(l), "hour")
        ), 1]
      ), A = [
        {
          id: "b",
          type: "line",
          data: i,
          areaStyle: {
            color: "rgba(160,210,255,0.14)"
          },
          lineStyle: {
            color: "#5CB0FE",
            width: 1
          },
          symbol: "none",
          markLine: X(i),
          z: 100,
          silent: !0,
          animation: !1
        },
        {
          id: "c",
          type: "bar",
          data: o,
          barWidth: 20,
          itemStyle: {
            color: "#C2E2FF",
            borderRadius: [3, 3, 0, 0]
          },
          z: 0,
          silent: !0,
          animation: !1
        }
      ];
      return {
        tooltip: {
          triggerOn: "none",
          formatter: function(l) {
            const d = p.value[Math.round(l.data[0])];
            return m(d).format("MM/DD HH:mm");
          }
        },
        grid: {
          top: "50%",
          left: "50",
          right: "50"
        },
        xAxis: {
          min: 0,
          max: r.value,
          interval: t.autoInterval ? f.value : t.interval,
          type: "value",
          // boundaryGap: "0%",
          boundaryGap: ["0%", "100%"],
          axisLine: { onZero: !1 },
          axisTick: { inside: !0 },
          splitLine: { show: !1 },
          axisLabel: {
            formatter: function(l) {
              let d = p.value[l];
              const c = m(d).hour();
              return c == 0 ? `{datebox|}{date|${m(d).format("MM/DD")}}` : `{time|${c}:00}`;
            },
            rich: {
              datebox: {
                width: 40
              },
              date: {
                color: "#333333",
                backgroundColor: "rgba(51,51,51,0.08)",
                padding: 4,
                borderRadius: 10
              },
              time: {
                color: "#666"
              }
            }
          }
        },
        yAxis: {
          type: "value",
          axisLine: { show: !1 },
          axisLabel: { show: !1 },
          axisTick: { show: !1 },
          splitLine: { show: !1 }
        },
        series: [
          {
            id: "a",
            type: "line",
            smooth: !0,
            data: a.value,
            areaStyle: {},
            symbolSize: 0
          },
          ...A
        ]
      };
    }, X = (e) => ({
      symbol: "none",
      animation: !1,
      z: 0,
      lineStyle: {
        color: "#5CB0FE",
        width: 1,
        type: "solid"
      },
      data: [
        // 左侧竖线
        [
          { coord: [e[0][0], 0] },
          { coord: [e[0][0], e[0][1]] }
        ],
        // 右侧竖线
        [
          { coord: [e[e.length - 1][0], 0] },
          {
            coord: [
              e[e.length - 1][0],
              e[e.length - 1][1]
            ]
          }
        ]
        // 底部横线
        // [
        //   { coord: [initialPoints[0][0], 0] },
        //   { coord: [initialPoints[initialPoints.length - 1][0], 0] },
        // ],
      ]
    }), P = (e) => {
      const n = [e.offsetX, e.offsetY], i = s.convertFromPixel(
        { seriesIndex: 0 },
        n
      );
      if (!i || i.length < 2) return;
      const o = Math.round(
        Math.min(Math.max(i[0], 0), r.value)
      ), A = Math.min(a.value[0][0], a.value[1][0]), l = Math.max(a.value[0][0], a.value[1][0]), d = Math.abs(o - A), c = Math.abs(o - l), D = d <= c, B = a.value[0][0] <= a.value[1][0] ? 0 : 1, V = B === 0 ? 1 : 0;
      let b = A, R = l;
      if (D) {
        const U = l - t.minRange, O = Math.max(0, l - t.maxRange);
        b = Math.max(O, Math.min(o, U));
      } else {
        const U = A + t.minRange, O = Math.min(r.value, A + t.maxRange);
        R = Math.min(O, Math.max(o, U));
      }
      const { leftX: E, rightX: k } = z(
        b,
        R
      );
      a.value[B] = [E, 0], a.value[V] = [k, 0], M(), Q();
    };
    return G(() => {
      s = se.init(g.value, null, {
        renderer: "svg"
      }), s.setOption(L()), setTimeout(() => {
        s.setOption({
          graphic: a.value.map((e, n) => ({
            type: "image",
            position: s.convertToPixel("grid", e),
            style: {
              image: n === 0 ? t.startIcon : t.endIcon,
              width: t.symbolSize,
              height: t.symbolSize,
              x: -t.symbolSize / 2,
              y: -t.symbolSize / 2
            },
            invisible: !1,
            draggable: "horizontal",
            ondrag: function(i, o) {
              Y(n, [this.x, this.y]);
            },
            ondragend: function() {
              N(n, [this.x, this.y]);
            },
            onmousemove: function() {
              K(n);
            },
            onmouseout: function() {
              F();
            },
            z: 200
          }))
        });
      }, 0), window.addEventListener("resize", C), s.on("dataZoom", C), t.needClick && s.getZr().on("click", P), g.value.addEventListener("wheel", q, { passive: !1 });
    }), ae(() => {
      window.removeEventListener("resize", C), s.off("dataZoom", C), s.getZr().off("click", P), g.value?.removeEventListener("wheel", q);
    }), H(
      () => t.timeRange,
      (e) => {
        console.log("newValue", e), s && (s.setOption(L()), C());
      },
      { immediate: !0 }
    ), (e, n) => (ie(), ne("div", pe, [
      oe("div", {
        ref_key: "chartRef",
        ref: g,
        class: "chart-wrap"
      }, null, 512)
    ]));
  }
}), Ie = (u, v) => {
  const t = u.__vccOpts || u;
  for (const [x, g] of v)
    t[x] = g;
  return t;
}, he = /* @__PURE__ */ Ie(Re, [["__scopeId", "data-v-05f757aa"]]), fe = [he], Ee = (u) => {
  fe.forEach((v) => {
    u.component(v.name, v);
  });
}, Me = { install: Ee };
export {
  he as DragChart,
  Me as default
};
