import { getCurrentScope as k, onScopeDispose as j, computed as h, toValue as S, watch as H, shallowRef as _, getCurrentInstance as $, onMounted as G, defineComponent as J, ref as ee, onBeforeUnmount as te, createElementBlock as ae, openBlock as ne, createElementVNode as ie } from "vue";
import m from "dayjs";
import * as oe from "echarts";
function se(c) {
  return k() ? (j(c), !0) : !1;
}
const re = typeof window < "u" && typeof document < "u";
typeof WorkerGlobalScope < "u" && globalThis instanceof WorkerGlobalScope;
const Ae = re ? window : void 0;
function Z(c) {
  var d;
  const t = S(c);
  return (d = t?.$el) != null ? d : t;
}
function le() {
  const c = _(!1), d = $();
  return d && G(() => {
    c.value = !0;
  }, d), c;
}
function ue(c) {
  const d = le();
  return h(() => (d.value, !!c()));
}
function ce(c, d, t = {}) {
  const { window: x = Ae, ...p } = t;
  let s;
  const A = ue(() => x && "ResizeObserver" in x), g = () => {
    s && (s.disconnect(), s = void 0);
  }, a = h(() => {
    const R = S(c);
    return Array.isArray(R) ? R.map((f) => Z(f)) : [Z(R)];
  }), Q = H(
    a,
    (R) => {
      if (g(), A.value && x) {
        s = new ResizeObserver(d);
        for (const f of R)
          f && s.observe(f, p);
      }
    },
    { immediate: !0, flush: "post" }
  ), B = () => {
    g(), Q();
  };
  return se(B), {
    isSupported: A,
    stop: B
  };
}
const ve = "data:image/png;base64,", de = "data:image/png;base64,", me = { class: "chart-container" }, ge = J({
  name: "DragChart"
}), pe = /* @__PURE__ */ J({
  ...ge,
  props: {
    // x轴的开始和结束时间
    timeRange: {
      type: Array,
      default: () => [m().subtract(1, "day"), m()]
    },
    // 开始图标
    startIcon: {
      type: String,
      default: ve
    },
    // 结束图标
    endIcon: {
      type: String,
      default: de
    },
    // 拖拽点的大小
    symbolSize: {
      type: Number,
      default: 20
    },
    // 目前是有值的柱子
    valueData: {
      type: Array,
      default: () => []
    },
    // 当前时间范围
    activeTime: {
      type: Array,
      default: () => [0, 12]
    },
    // X轴的间隔
    interval: {
      type: Number,
      default: 4
    },
    // 是否自动计算间隔, 如果为true，则interval会被忽略
    autoInterval: {
      type: Boolean,
      default: !0
    },
    // 是否支持点击修改位置
    needClick: {
      type: Boolean,
      default: !0
    },
    // 最大选择范围（小时）
    maxRange: {
      type: Number,
      default: 72
      // 3天 = 72小时
    },
    // 最小选择范围（小时）
    minRange: {
      type: Number,
      default: 3
      // 3小时
    }
  },
  emits: ["update:activeTime"],
  setup(c, { emit: d }) {
    const t = c, x = d, p = ee();
    let s;
    const A = h(() => 24 * t.timeRange.length - 1), g = h(() => {
      const e = [], n = m(t.timeRange[0]), o = m(t.timeRange[t.timeRange.length - 1]).diff(n, "day");
      for (let r = 0; r <= o; r++) {
        const u = n.add(r, "day");
        for (let l = 0; l < 24; l++)
          e.push(u.hour(l).format("YYYY-MM-DD HH:00:00"));
      }
      return e;
    }), a = h({
      get() {
        return Q();
      },
      set(e) {
        x("update:activeTime", e);
      }
    }), Q = () => {
      if (!t.activeTime || t.activeTime.length !== 2)
        return [
          [0, 0],
          [g.value.length - 1, 0]
        ];
      const e = g.value.findIndex(
        (i) => m(i).isSame(m(t.activeTime[0]), "hour")
      ), n = g.value.findIndex(
        (i) => m(i).isSame(m(t.activeTime[1]), "hour")
      );
      return [
        [e, 0],
        [n, 0]
      ];
    };
    ce(p, () => {
      s && s.resize({
        width: "auto",
        height: "auto"
      });
    });
    const R = h(() => Math.floor((A.value - 1) / 12) + 1), f = h(() => !t.valueData || t.valueData.length === 0 ? [] : t.valueData.reduce((n, i) => {
      const o = n[n.length - 1];
      return o && m(o).hour() === m(i).hour() ? n : [
        ...n,
        m(i).set("minute", 0).set("second", 0).format("YYYY-MM-DD HH:mm:ss")
      ];
    }, [])), W = (e, n) => {
      const i = [];
      for (let o = e; o <= n; o += 1)
        i.push([o, 2]);
      return i;
    }, z = (e, n) => {
      const i = Math.abs(n - e);
      if (i < t.minRange) {
        const o = (e + n) / 2, r = t.minRange / 2;
        e = Math.max(0, o - r), n = Math.min(A.value, o + r), n - e < t.minRange && (e === 0 ? n = Math.min(A.value, t.minRange) : n === A.value && (e = Math.max(0, A.value - t.minRange)));
      }
      if (i > t.maxRange) {
        const o = (e + n) / 2, r = t.maxRange / 2;
        e = Math.max(0, o - r), n = Math.min(A.value, o + r), e === 0 && n - e > t.maxRange ? n = t.maxRange : n === A.value && n - e > t.maxRange && (e = A.value - t.maxRange);
      }
      return {
        leftX: Math.round(e),
        rightX: Math.round(n)
      };
    }, y = () => {
      s.setOption({
        graphic: a.value.map((e) => ({
          position: s.convertToPixel("grid", e)
        }))
      });
    }, T = (e) => {
      s.dispatchAction({
        type: "showTip",
        seriesIndex: 0,
        dataIndex: e
      });
    }, K = () => {
      s.dispatchAction({
        type: "hideTip"
      });
    }, C = () => {
      const e = Math.min(a.value[0][0], a.value[1][0]), n = Math.max(a.value[0][0], a.value[1][0]), i = W(e, n), o = a.value[0][0], r = a.value[1][0], u = o < r;
      s.setOption({
        series: [
          {
            id: "a",
            data: a.value,
            z: 80,
            silent: !0,
            animation: !1,
            symbolSize: 0
          },
          {
            id: "b",
            data: i,
            type: "line",
            areaStyle: {
              color: "rgba(160,210,255,0.14)"
            },
            symbolSize: 0,
            markLine: X(i),
            z: 90,
            silent: !0,
            animation: !1
          }
        ]
      }), s.setOption({
        graphic: a.value.map((l, v) => ({
          position: s.convertToPixel("grid", l),
          type: "image",
          style: {
            image: (u ? v === 0 : v === 1) ? t.startIcon : t.endIcon,
            width: t.symbolSize,
            height: t.symbolSize,
            x: -t.symbolSize / 2,
            y: -t.symbolSize / 2
          },
          invisible: !1,
          z: 100
        }))
      });
    }, b = () => {
      x(
        "update:activeTime",
        a.value.map((e) => g.value[e[0]])
      );
    }, F = (e, n) => {
      const i = s.convertFromPixel("grid", n);
      i[0] = Math.round(Math.min(Math.max(i[0], 0), A.value)), i[1] = a.value[e][1], a.value[e] = i;
      const o = Math.min(a.value[0][0], a.value[1][0]), r = Math.max(a.value[0][0], a.value[1][0]), { leftX: u, rightX: l } = z(
        o,
        r
      ), v = a.value[0][0] <= a.value[1][0] ? 0 : 1, M = v === 0 ? 1 : 0;
      a.value[v] = [u, 0], a.value[M] = [l, 0], C(), b();
    }, N = (e, n) => {
      const i = s.convertFromPixel("grid", n);
      i[0] = Math.min(Math.max(i[0], 0), A.value), i[1] = a.value[e][1], a.value[e] = i, C();
    }, q = (e) => {
      if (!e.ctrlKey && !e.shiftKey) return;
      e.preventDefault();
      const n = e.deltaY, i = Math.abs(a.value[1][0] - a.value[0][0]);
      if (e.shiftKey) {
        const D = n > 0 ? 2 : -2;
        let I = a.value[0][0] + D, E = a.value[1][0] + D;
        I < 0 && (I = 0, E = i), E > A.value && (E = A.value, I = A.value - i), a.value[0] = [I, 0], a.value[1] = [E, 0], C(), b();
        return;
      }
      const o = 0.1;
      if (i <= t.minRange && n > 0 || i >= t.maxRange && n < 0) return;
      let r = n > 0 ? Math.min(
        t.maxRange,
        Math.min(A.value, i * (1 + o))
      ) : Math.max(t.minRange, i * (1 - o));
      const u = (a.value[0][0] + a.value[1][0]) / 2;
      let l = Math.round(u - r / 2), v = Math.round(u + r / 2);
      l < 0 && (l = 0, v = Math.min(A.value, Math.round(r))), v > A.value && (v = A.value, l = Math.max(0, Math.round(A.value - r)));
      const { leftX: M, rightX: w } = z(
        l,
        v
      );
      a.value[0] = [M, 0], a.value[1] = [w, 0], C(), b();
    }, L = () => {
      const e = Math.min(a.value[0][0], a.value[1][0]), n = Math.max(a.value[0][0], a.value[1][0]), i = W(e, n), o = f.value.map(
        (u) => [g.value.findIndex(
          (v) => m(v).isSame(m(u), "hour")
        ), 1]
      ), r = [
        {
          id: "b",
          type: "line",
          data: i,
          areaStyle: {
            color: "rgba(160,210,255,0.14)"
          },
          lineStyle: {
            color: "#5CB0FE",
            width: 1
          },
          symbol: "none",
          markLine: X(i),
          z: 100,
          silent: !0,
          animation: !1
        },
        {
          id: "c",
          type: "bar",
          data: o,
          barWidth: 20,
          itemStyle: {
            color: "#C2E2FF",
            borderRadius: [3, 3, 0, 0]
          },
          z: 0,
          silent: !0,
          animation: !1
        }
      ];
      return {
        tooltip: {
          triggerOn: "none",
          formatter: function(u) {
            const l = g.value[Math.round(u.data[0])];
            return m(l).format("MM/DD HH:mm");
          }
        },
        grid: {
          top: "50%",
          left: "50",
          right: "50"
        },
        xAxis: {
          min: 0,
          max: A.value,
          interval: t.autoInterval ? R.value : t.interval,
          type: "value",
          // boundaryGap: "0%",
          boundaryGap: ["0%", "100%"],
          axisLine: { onZero: !1 },
          axisTick: { inside: !0 },
          splitLine: { show: !1 },
          axisLabel: {
            formatter: function(u) {
              let l = g.value[u];
              const v = m(l).hour();
              return v == 0 ? `{datebox|}{date|${m(l).format("MM/DD")}}` : `{time|${v}:00}`;
            },
            rich: {
              datebox: {
                width: 40
              },
              date: {
                color: "#333333",
                backgroundColor: "rgba(51,51,51,0.08)",
                padding: 4,
                borderRadius: 10
              },
              time: {
                color: "#666"
              }
            }
          }
        },
        yAxis: {
          type: "value",
          axisLine: { show: !1 },
          axisLabel: { show: !1 },
          axisTick: { show: !1 },
          splitLine: { show: !1 }
        },
        series: [
          {
            id: "a",
            type: "line",
            smooth: !0,
            data: a.value,
            areaStyle: {},
            symbolSize: 0
          },
          ...r
        ]
      };
    }, X = (e) => ({
      symbol: "none",
      animation: !1,
      z: 0,
      lineStyle: {
        color: "#5CB0FE",
        width: 1,
        type: "solid"
      },
      data: [
        // 左侧竖线
        [
          { coord: [e[0][0], 0] },
          { coord: [e[0][0], e[0][1]] }
        ],
        // 右侧竖线
        [
          { coord: [e[e.length - 1][0], 0] },
          {
            coord: [
              e[e.length - 1][0],
              e[e.length - 1][1]
            ]
          }
        ]
        // 底部横线
        // [
        //   { coord: [initialPoints[0][0], 0] },
        //   { coord: [initialPoints[initialPoints.length - 1][0], 0] },
        // ],
      ]
    }), V = (e) => {
      const n = [e.offsetX, e.offsetY], i = s.convertFromPixel(
        { seriesIndex: 0 },
        n
      );
      if (!i || i.length < 2) return;
      const o = Math.round(
        Math.min(Math.max(i[0], 0), A.value)
      ), r = Math.min(a.value[0][0], a.value[1][0]), u = Math.max(a.value[0][0], a.value[1][0]), l = Math.abs(o - r), v = Math.abs(o - u), M = l <= v, w = a.value[0][0] <= a.value[1][0] ? 0 : 1, P = w === 0 ? 1 : 0;
      let D = r, I = u;
      if (M) {
        const U = u - t.minRange, O = Math.max(0, u - t.maxRange);
        D = Math.max(O, Math.min(o, U));
      } else {
        const U = r + t.minRange, O = Math.min(A.value, r + t.maxRange);
        I = Math.min(O, Math.max(o, U));
      }
      const { leftX: E, rightX: Y } = z(
        D,
        I
      );
      a.value[w] = [E, 0], a.value[P] = [Y, 0], C(), b();
    };
    return G(() => {
      s = oe.init(p.value, null, {
        renderer: "svg"
      }), s.setOption(L()), setTimeout(() => {
        s.setOption({
          graphic: a.value.map((e, n) => ({
            type: "image",
            position: s.convertToPixel("grid", e),
            style: {
              image: n === 0 ? t.startIcon : t.endIcon,
              width: t.symbolSize,
              height: t.symbolSize,
              x: -t.symbolSize / 2,
              y: -t.symbolSize / 2
            },
            invisible: !1,
            draggable: "horizontal",
            ondrag: function(i, o) {
              N(n, [this.x, this.y]);
            },
            ondragend: function() {
              F(n, [this.x, this.y]);
            },
            onmousemove: function() {
              T(n);
            },
            onmouseout: function() {
              K();
            },
            z: 200
          }))
        });
      }, 0), window.addEventListener("resize", y), s.on("dataZoom", y), t.needClick && s.getZr().on("click", V), p.value.addEventListener("wheel", q, { passive: !1 });
    }), te(() => {
      window.removeEventListener("resize", y), s.off("dataZoom", y), s.getZr().off("click", V), p.value?.removeEventListener("wheel", q);
    }), H(
      () => t.timeRange,
      (e) => {
        s && (s.setOption(L()), y());
      },
      { immediate: !0 }
    ), (e, n) => (ne(), ae("div", me, [
      ie("div", {
        ref_key: "chartRef",
        ref: p,
        class: "chart-wrap"
      }, null, 512)
    ]));
  }
}), xe = (c, d) => {
  const t = c.__vccOpts || c;
  for (const [x, p] of d)
    t[x] = p;
  return t;
}, Re = /* @__PURE__ */ xe(pe, [["__scopeId", "data-v-1d86c1a1"]]), Ie = [Re], he = (c) => {
  Ie.forEach((d) => {
    c.component(d.name, d);
  });
}, ye = { install: he };
export {
  Re as DragChart,
  ye as default
};
