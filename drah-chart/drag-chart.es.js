import { getCurrentScope as _, onScopeDispose as $, computed as I, toValue as S, watch as H, shallowRef as ee, getCurrentInstance as te, onMounted as G, defineComponent as J, ref as ne, onBeforeUnmount as ae, createElementBlock as ie, openBlock as oe, createElementVNode as se } from "vue";
import d from "dayjs";
import * as re from "echarts";
const le = "data:image/png;base64,", Ae = "data:image/png;base64,", ue = {
  // x轴的开始和结束时间
  timeRange: {
    type: Array,
    default: () => [d().subtract(1, "day"), d()]
  },
  // 开始图标
  startIcon: {
    type: String,
    default: le
  },
  // 结束图标
  endIcon: {
    type: String,
    default: Ae
  },
  // 拖拽点的大小
  symbolSize: {
    type: Number,
    default: 20
  },
  // 目前是有值的柱子
  valueData: {
    type: Array,
    default: () => []
  },
  // 当前时间范围
  activeTime: {
    type: Array,
    default: () => [0, 12]
  },
  // X轴的间隔
  interval: {
    type: Number,
    default: 4
  },
  // 是否自动计算间隔, 如果为true，则interval会被忽略
  autoInterval: {
    type: Boolean,
    default: !0
  },
  // 是否支持点击修改位置
  needClick: {
    type: Boolean,
    default: !0
  },
  // 最大选择范围（小时）
  maxRange: {
    type: Number,
    default: 7 * 24
    // 3天 = 72小时
  },
  // 最小选择范围（小时）
  minRange: {
    type: Number,
    default: 3
    // 3小时
  },
  // 覆盖颜色
  coverColor: {
    type: String,
    default: "rgba(160,210,255,0.14)"
    // 默认颜色
  },
  // 线段颜色
  lineColor: {
    type: String,
    default: "#5CB0FE"
    // 默认线条颜色
  }
};
function ce(u) {
  return _() ? ($(u), !0) : !1;
}
const ve = typeof window < "u" && typeof document < "u";
typeof WorkerGlobalScope < "u" && globalThis instanceof WorkerGlobalScope;
const me = ve ? window : void 0;
function Z(u) {
  var v;
  const t = S(u);
  return (v = t?.$el) != null ? v : t;
}
function de() {
  const u = ee(!1), v = te();
  return v && G(() => {
    u.value = !0;
  }, v), u;
}
function ge(u) {
  const v = de();
  return I(() => (v.value, !!u()));
}
function Re(u, v, t = {}) {
  const { window: g = me, ...R } = t;
  let s;
  const E = ge(() => g && "ResizeObserver" in g), r = () => {
    s && (s.disconnect(), s = void 0);
  }, p = I(() => {
    const f = S(u);
    return Array.isArray(f) ? f.map((h) => Z(h)) : [Z(f)];
  }), n = H(
    p,
    (f) => {
      if (r(), E.value && g) {
        s = new ResizeObserver(v);
        for (const h of f)
          h && s.observe(h, R);
      }
    },
    { immediate: !0, flush: "post" }
  ), b = () => {
    r(), n();
  };
  return ce(b), {
    isSupported: E,
    stop: b
  };
}
const pe = { class: "chart-container" }, xe = J({
  name: "DragChart"
}), Ie = /* @__PURE__ */ J({
  ...xe,
  props: {
    ...ue
  },
  emits: ["update:activeTime", "outOfRange"],
  setup(u, { emit: v }) {
    const t = u, g = v, R = ne();
    let s;
    const E = (e = t.timeRange[0], a = t.timeRange[t.timeRange.length - 1]) => d(a).diff(e, "day") + 1, r = I(() => E() * 24), p = I(() => {
      const e = [], a = d(t.timeRange[0]), i = E();
      for (let o = 0; o <= i; o++) {
        const l = a.add(o, "day");
        for (let A = 0; A < 24; A++)
          e.push(l.hour(A).format("YYYY-MM-DD HH:00:00"));
      }
      return e;
    }), n = I({
      get() {
        return b();
      },
      set(e) {
        g("update:activeTime", e);
      }
    }), b = () => {
      if (!t.activeTime || t.activeTime.length !== 2)
        return [
          [0, 0],
          [p.value.length - 1, 0]
        ];
      const e = p.value.findIndex(
        (i) => d(i).isSame(
          d(t.activeTime[0]),
          "hour"
        )
      ), a = p.value.findIndex(
        (i) => d(i).isSame(
          d(t.activeTime[1]),
          "hour"
        )
      );
      return [
        [e, 0],
        [a, 0]
      ];
    };
    Re(R, () => {
      s && s.resize({
        width: "auto",
        height: "auto"
      });
    });
    const h = I(() => Math.floor((r.value - 1) / 12) + 1), T = I(() => !t.valueData || t.valueData.length === 0 ? [] : t.valueData.reduce((a, i) => {
      const o = a[a.length - 1];
      return o && d(o).hour() === d(i).hour() ? a : [
        ...a,
        d(i).set("minute", 0).set("second", 0).format("YYYY-MM-DD HH:mm:ss")
      ];
    }, [])), W = (e, a) => {
      const i = [];
      for (let o = e; o <= a; o += 1)
        i.push([o, 2]);
      return i;
    }, B = (e, a) => {
      const i = Math.abs(a - e);
      if (i < t.minRange) {
        const o = (e + a) / 2, l = t.minRange / 2;
        e = Math.max(0, o - l), a = Math.min(r.value, o + l), a - e < t.minRange ? e === 0 ? a = Math.min(r.value, t.minRange) : a === r.value && (e = Math.max(0, r.value - t.minRange)) : g("outOfRange", {
          type: "min",
          currentRange: i,
          minRange: t.minRange
        });
      }
      if (i > t.maxRange) {
        const o = (e + a) / 2, l = t.maxRange / 2;
        e = Math.max(0, o - l), a = Math.min(r.value, o + l), e === 0 && a - e > t.maxRange ? a = t.maxRange : a === r.value && a - e > t.maxRange ? e = r.value - t.maxRange : (console.log(`当前范围: ${i}, 最大范围: ${t.maxRange}`), g("outOfRange", {
          type: "max",
          currentRange: i,
          minRange: t.minRange
        }));
      }
      return {
        leftX: Math.round(e),
        rightX: Math.round(a)
      };
    }, z = () => {
      s.setOption({
        graphic: n.value.map((e) => ({
          position: s.convertToPixel("grid", e)
        }))
      });
    }, K = (e) => {
      s.dispatchAction({
        type: "showTip",
        seriesIndex: 0,
        dataIndex: e
      });
    }, F = () => {
      s.dispatchAction({
        type: "hideTip"
      });
    }, C = () => {
      const e = Math.min(n.value[0][0], n.value[1][0]), a = Math.max(n.value[0][0], n.value[1][0]), i = W(e, a), o = n.value[0][0], l = n.value[1][0], A = o < l;
      s.setOption({
        series: [
          {
            id: "a",
            data: n.value,
            z: 80,
            silent: !0,
            animation: !1,
            symbolSize: 0
          },
          {
            id: "lineRange",
            data: i,
            type: "line",
            areaStyle: {
              color: t.coverColor
            },
            symbolSize: 0,
            markLine: X(i),
            z: 90,
            silent: !0,
            animation: !1
          }
        ]
      }), s.setOption({
        graphic: n.value.map((m, c) => ({
          position: s.convertToPixel("grid", m),
          type: "image",
          style: {
            image: (A ? c === 0 : c === 1) ? t.startIcon : t.endIcon,
            width: t.symbolSize,
            height: t.symbolSize,
            x: -t.symbolSize / 2,
            y: -t.symbolSize / 2
          },
          invisible: !1,
          z: 100
        }))
      });
    }, w = () => {
      g(
        "update:activeTime",
        n.value.map((e) => p.value[e[0]])
      );
    }, N = (e, a) => {
      const i = s.convertFromPixel("grid", a);
      i[0] = Math.round(Math.min(Math.max(i[0], 0), r.value)), i[1] = n.value[e][1], n.value[e] = i;
      const o = Math.min(n.value[0][0], n.value[1][0]), l = Math.max(n.value[0][0], n.value[1][0]), { leftX: A, rightX: m } = B(
        o,
        l
      ), c = n.value[0][0] <= n.value[1][0] ? 0 : 1, M = c === 0 ? 1 : 0;
      n.value[c] = [A, 0], n.value[M] = [m, 0], C(), w();
    }, Y = (e, a) => {
      const i = s.convertFromPixel("grid", a);
      i[0] = Math.min(Math.max(i[0], 0), r.value), i[1] = n.value[e][1], n.value[e] = i, C();
    }, q = (e) => {
      if (!e.ctrlKey && !e.shiftKey) return;
      e.preventDefault();
      const a = e.deltaY, i = Math.abs(n.value[1][0] - n.value[0][0]);
      if (e.shiftKey) {
        const D = a > 0 ? 2 : -2;
        let x = n.value[0][0] + D, y = n.value[1][0] + D;
        x < 0 && (x = 0, y = i), y > r.value && (y = r.value, x = r.value - i), n.value[0] = [x, 0], n.value[1] = [y, 0], C(), w();
        return;
      }
      const o = 0.1;
      if (i <= t.minRange && a > 0 || i >= t.maxRange && a < 0) return;
      let l = a > 0 ? Math.min(
        t.maxRange,
        Math.min(r.value, i * (1 + o))
      ) : Math.max(t.minRange, i * (1 - o));
      const A = (n.value[0][0] + n.value[1][0]) / 2;
      let m = Math.round(A - l / 2), c = Math.round(A + l / 2);
      m < 0 && (m = 0, c = Math.min(r.value, Math.round(l))), c > r.value && (c = r.value, m = Math.max(0, Math.round(r.value - l)));
      const { leftX: M, rightX: Q } = B(
        m,
        c
      );
      n.value[0] = [M, 0], n.value[1] = [Q, 0], C(), w();
    }, L = () => {
      const e = Math.min(n.value[0][0], n.value[1][0]), a = Math.max(n.value[0][0], n.value[1][0]), i = W(e, a), o = T.value.map(
        (A) => [p.value.findIndex(
          (c) => d(c).isSame(d(A), "hour")
        ), 1]
      ), l = [
        {
          id: "lineRange",
          type: "line",
          data: i,
          areaStyle: {
            color: t.coverColor
          },
          lineStyle: {
            color: t.lineColor,
            width: 1
          },
          symbol: "none",
          markLine: X(i),
          z: 100,
          silent: !0,
          animation: !1
        },
        {
          id: "barData",
          type: "bar",
          data: o,
          barWidth: 20,
          itemStyle: {
            color: "#C2E2FF",
            borderRadius: [3, 3, 0, 0]
          },
          z: 0,
          silent: !0,
          animation: !1
        }
      ];
      return {
        tooltip: {
          triggerOn: "none",
          formatter: function(A) {
            const m = p.value[Math.round(A.data[0])];
            return d(m).format("MM/DD HH:mm");
          }
        },
        grid: {
          top: "50%",
          left: "50",
          right: "50"
        },
        xAxis: {
          min: 0,
          max: r.value,
          interval: t.autoInterval ? h.value : t.interval,
          type: "value",
          // boundaryGap: "0%",
          boundaryGap: ["0%", "100%"],
          axisLine: { onZero: !1 },
          axisTick: { inside: !0 },
          splitLine: { show: !1 },
          axisLabel: {
            formatter: function(A) {
              let m = p.value[A];
              const c = d(m).hour();
              return c == 0 ? `{datebox|}{date|${d(m).format("MM/DD")}}` : `{time|${c}:00}`;
            },
            rich: {
              datebox: {
                width: 40
              },
              date: {
                color: "#333333",
                backgroundColor: "rgba(51,51,51,0.08)",
                padding: 4,
                borderRadius: 10
              },
              time: {
                color: "#666"
              }
            }
          }
        },
        yAxis: {
          type: "value",
          axisLine: { show: !1 },
          axisLabel: { show: !1 },
          axisTick: { show: !1 },
          splitLine: { show: !1 }
        },
        series: [
          {
            id: "a",
            type: "line",
            smooth: !0,
            data: n.value,
            areaStyle: {},
            symbolSize: 0
          },
          ...l
        ]
      };
    }, X = (e) => ({
      symbol: "none",
      animation: !1,
      z: 0,
      lineStyle: {
        color: t.lineColor,
        width: 1,
        type: "solid"
      },
      data: [
        // 左侧竖线
        [
          { coord: [e[0][0], 0] },
          { coord: [e[0][0], e[0][1]] }
        ],
        // 右侧竖线
        [
          { coord: [e[e.length - 1][0], 0] },
          {
            coord: [
              e[e.length - 1][0],
              e[e.length - 1][1]
            ]
          }
        ]
        // 底部横线
        // [
        //   { coord: [initialPoints[0][0], 0] },
        //   { coord: [initialPoints[initialPoints.length - 1][0], 0] },
        // ],
      ]
    }), P = (e) => {
      const a = [e.offsetX, e.offsetY], i = s.convertFromPixel(
        { seriesIndex: 0 },
        a
      );
      if (!i || i.length < 2) return;
      const o = Math.round(
        Math.min(Math.max(i[0], 0), r.value)
      ), l = Math.min(n.value[0][0], n.value[1][0]), A = Math.max(n.value[0][0], n.value[1][0]), m = Math.abs(o - l), c = Math.abs(o - A), M = m <= c, Q = n.value[0][0] <= n.value[1][0] ? 0 : 1, V = Q === 0 ? 1 : 0;
      let D = l, x = A;
      if (M) {
        const U = A - t.minRange, O = Math.max(0, A - t.maxRange);
        D = Math.max(O, Math.min(o, U));
      } else {
        const U = l + t.minRange, O = Math.min(r.value, l + t.maxRange);
        x = Math.min(O, Math.max(o, U));
      }
      const { leftX: y, rightX: j } = B(
        D,
        x
      );
      n.value[Q] = [y, 0], n.value[V] = [j, 0], C(), w();
    }, k = () => {
      s.setOption({
        graphic: n.value.map((e, a) => ({
          type: "image",
          position: s.convertToPixel("grid", e),
          style: {
            image: a === 0 ? t.startIcon : t.endIcon,
            width: t.symbolSize,
            height: t.symbolSize,
            x: -t.symbolSize / 2,
            y: -t.symbolSize / 2
          },
          invisible: !1,
          draggable: "horizontal",
          ondrag: function(i, o) {
            Y(a, [this.x, this.y]);
          },
          ondragend: function() {
            N(a, [this.x, this.y]);
          },
          onmousemove: function() {
            K(a);
          },
          onmouseout: function() {
            F();
          },
          z: 200
        }))
      });
    };
    return G(() => {
      s = re.init(R.value, null, {
        renderer: "svg"
      }), s.setOption(L()), setTimeout(() => {
        k();
      }, 0), window.addEventListener("resize", z), t.needClick && s.getZr().on("click", P), R.value.addEventListener("wheel", q, { passive: !1 });
    }), ae(() => {
      window.removeEventListener("resize", z), s.getZr().off("click", P), R.value?.removeEventListener("wheel", q);
    }), H(
      () => t.timeRange,
      (e) => {
        s && (s.setOption(L()), z());
      },
      { immediate: !0 }
    ), (e, a) => (oe(), ie("div", pe, [
      se("div", {
        ref_key: "chartRef",
        ref: R,
        class: "chart-wrap"
      }, null, 512)
    ]));
  }
}), fe = (u, v) => {
  const t = u.__vccOpts || u;
  for (const [g, R] of v)
    t[g] = R;
  return t;
}, he = /* @__PURE__ */ fe(Ie, [["__scopeId", "data-v-45718270"]]), ye = [he], Ee = (u) => {
  ye.forEach((v) => {
    u.component(v.name, v);
  });
}, De = { install: Ee };
export {
  he as DragChart,
  De as default
};
